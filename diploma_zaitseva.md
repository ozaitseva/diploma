...
Процессоры, которые поддерживают системы Windows, поддерживают страницы трех размеров — малые, размером 4 КиБ, большие, размером 2 МиБ и огромные, размером 1 ГиБ [7]. По умолчанию адресное пространство каждого процесса изолировано, то есть не может возникнуть такой ситуации, что данные двух разных процессов будут записаны в одну и ту же страницу физической памяти. За преобразование виртуального адреса в физический отвечают таблицы страниц, которые каждой виртуальной странице ставит в соответствие страничный кадр.

Для Windows на платформе x86 возможны две схемы преобразования адреса: с расширением физических адресов (Physical Address Extension, PAE) и без него. Системы на платформе x86 без
