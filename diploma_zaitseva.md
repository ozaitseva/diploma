...
В ячейке хранятся разделы, параметры, список подразделов и параметров раздела. Ячейка может быть одного из пяти типов:
