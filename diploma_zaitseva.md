...
# Глава 1. Обзор существующих методик и технологий

На данный момент для извлечения и анализа данных физической памяти компьютеров, работающих под управлением операционных систем Windows NT существует большое количество программно-аппаратных средств. Однако не все данные средства могут быть использованы при проведении исследований в области компьютерной криминалистики, поэтому все методы обработки цифровой информации можно разделить на:

* криминалистически правильные;
* криминалистически сомнительные;
*	криминалистически неправильные. 

Также можно ввести классификацию, связанную со способом исследования оборудования, а именно:

*	методы, не требующие остановки целевой системы;
*	методы, приостанавливающие целевую систему.

Наконец, среди всех методов могут быть выделены методы, которые модифицируют память целевой систему и методы, которые, наоборот, не содержат в памяти свой резидентный код, тем самым не оставляю следов. Данные методы можно соответственно классифицировать как:
...
*	 
К программным методам относятся такие утилиты, как Belkasoft Live RAM Caputer, MANDIANT Memoryze, HBGary, FTK Imager, Volatility Framework и др. Главный недостаток всех программных методов заключается в их инвазивности, и как следствие, они являются криминалистически неправильные. Также стоит отметить, что программные средства являются труднодоступными, поддерживают не все форматы дампов оперативной памяти компьютера, имеют высокую стоимость и закрытый исходный код, что не позволяет их модифицировать под конкретные задачи. 

## Настройка рабочего окружения (*//? к какой главе отнести?*)
...
# Глава 2. Обзор устройства внутренней памяти Windows 7 на платформе x86-64. Преобразование виртуального адреса в физический
...
архитектуре x86-64 теоретически может составлять 16 экзабайт (2^64байт) [6]. Понятно, что у современных компьютеров нет необходимости поддерживать такой объем
...
Процессоры, на которых работает Windows, поддерживают страницы двух размеров — малые, размером 4 Кб, и большие, размером 2 Мб. [7] По умолчанию адресное
...
Они используют четырехуровневую структуру таблицы страниц. 48-разрядный виртуальный адрес представляет собой совокупность 5 полей (см. Рис 1.):
...
селектор записи таблицы страниц (биты 20-12) и байтовое смещение (биты 11-0). Эти поля служат для локализации записей в соответствующих таблицах преобразования [6].
...
Записи (такие как PDPE, PDE, PTE и др) в нижеописанных таблицах преобразования представляют собой 8-байтные значения (см. Рис 3.). Биты с номерами 12-39 образуют
...
Каждый процесс ОС Windows представлен структурой *\_EPROCESS*. Данная структура содержит атрибуты, описывающие процесс, а также другие связанные с ним данные (например, структуру *\_ ETHREAD*, описывающую потоки процесса).

Windows использует круговой двусвязный список структур EPROCESS для отслеживания процесса исполнения (см. Рис. 4.). Ссылки, связывающие объекты EPROCESS, содержатся в поле ActiveProcessLink, структура которого LIST_ENTRY.
```
...
Структуры *\_EPROCESS* располагаются в адресном пространстве ядра ОС.
...
поэтому необходимо проводить проверку. Именно, следует проверять, что последующий и предыдущий потоки процесса находятся в виртуальном адресном пространстве ядра [8]:
...
Реестр в Windows - иерархически построенная база данных параметров и настроек в большинстве операционных систем Microsoft Windows [9]. Он используется приложениями
...
В ячейке хранятся разделы, параметры, список подразделов и параметров раздела. Ячейка может быть одного из пяти типов [9]:
...
отображения адресов виртуальной памяти в физические адреса. Для отображения ячеек индекс ячейки делится на три поля (см. Рис. 4.):
...
ячеек хранится в подструктуре структуры *\_HHIVE* - *Storage[2]* в поле *Map* [9].
...
В контексте создания криминалистического программного комплекса, позволяющего производить неинвазивный анализ динамики оперативной памяти, необходимо автоматизировать вышеописанные алгоритмы: алгоритм поиска данных процесса, реестра и алгоритм трансляции виртуального адреса в физический. Схематично архитектуру решения можно представить следующим образом (см. Рис. 5.):

![Архитектура решения](https://github.com/ozaitseva/diploma/blob/master/images/project_plan.jpg "Архитектура решения")
...
В ходе эксперимента было выявлено, что на гостевой операционной системе, дамп физической памяти которой был исследован, запущены следующие процессы [4]:

**Таблица 2.** Список найденных процессов
...
Также с помощью реализованного алгоритма удалось установить список файлов реестра, а именно имя файла реестра, физический адрес хайва, и удалось осуществить трансляцию индекса ячейки каждого хайва с последующей целью обхода узлов реестра. (//? обход не получается осуществить, не могу разобраться, что за странный указатель на subkeys, как лучше это описать?)

**Таблица 3.** Список найденных файлов реестра
...
На данном этапе были реализованы методы обработки процессов и данных реестра дампов оперативной памяти ОС Windows 7. В отличие от существующих методик и технологий имплементированные алгоритмы подходят для обработки любых форматов дампов физической памяти.

В дальнейшем планируется создать автоматизированный программный комплекс, задача которого состоит в неинвазивном анализе динамики оперативной памяти гостевых операционных систем. Разработанный алгоритм паттерн-анализа дампов является частью данного комплекса.
...
[8]	Schuster A. Searching for processes and threads in Microsoft Windows memory dumps // Digital Investigation. 2006. Vol. 3. P.10-16.

[9]	Russinovich M., Solomon D., Ionescu A. Windows Internals, Part 1, Sixth Edition. Redmond, Washington: Microsoft Press, 2012. 726 p.

[10]10.	Dolan-Gavitt B. Forensic analysis of the Windows registry in memory // Digital Investigation. 2008. Vol. 5. P.26-32.
