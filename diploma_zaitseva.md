...
* изучить устройство памяти операционных систем семейства Windows 7 X86_64;
...
Наконец, все методы могут быть разделены по признаку инвазивности:

*	инвазивные – методы, которые модифицируют память в той или иной степени;;
*	неинвазивные - методы, которые не требуют наличие в памяти своего резидентного кода, тем самым не оставляют следов..
...
Каждый процесс ОС Windows представлен структурой *\_EPROCESS*. Данная структура содержит атрибуты, описывающие процесс, а также другие связанные с ним данные (например, структуру *\_ETHREAD*, описывающую потоки процесса).
...
Для отображения ячеек индекс ячейки делится на три поля (см. Рис. 4.):
